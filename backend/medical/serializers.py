...
class AgentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # Vérifier que site, service et job_position appartiennent à l'entreprise
        company_obj = attrs.get('company') or (self.instance.company if self.instance else None)
        if company_obj:
            site = attrs.get('site')
            service = attrs.get('service')
            job_position = attrs.get('job_position')
            if site is not None and site and getattr(site, 'company_id', None) != company_obj.id:
                raise serializers.ValidationError({
                    'site': 'Ce site n\'appartient pas à l\'entreprise sélectionnée.'
                })
            if service is not None and service and getattr(service, 'company_id', None) != company_obj.id:
                raise serializers.ValidationError({
                    'service': 'Ce service n\'appartient pas à l\'entreprise sélectionnée.'
                })
            if job_position is not None and job_position and getattr(job_position, 'company_id', None) != company_obj.id:
                raise serializers.ValidationError({
